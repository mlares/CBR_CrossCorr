...
def Plot_map(x, Data, id_Sample , ymax, ymin):
	...
	plt.savefig(fileProject+'graficos/CMBMeanT_on_'+str(id_Sample)+'_z0003_z'+str(Samp_choice)+'_ba0_05.png')
...
if AB_fitl == True:
	Sa_sample = Sa_sample[ Sa_sample['b/a'] < 0.5 ]
	Sb_sample = Sb_sample[ Sb_sample['b/a'] < 0.5 ]
	Sc_sample = Sc_sample[ Sc_sample['b/a'] < 0.5 ]
...
#				R_disk_ext = (50.0)*arcsec_to_rad			# Radius of galaxy k in radians
			...
			print('')
			print('Percentage of the sky with data covered: ', len(list(Tpix_disks))/(0.78*Nside**2)  )
			print('################################################################')
												
			# We compute CMB T mean.
			T_val_ss = np.array(list(Tpix_disks))			
