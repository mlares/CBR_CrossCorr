...
def Plot_map(x, Data, id_Sample , ymax, ymin):
	plt.close()
	plt.figure(0)
	plt.errorbar( x, np.array(Data[0][0])*K_to_muK,\
				 yerr=np.array(Data[0][1])*K_to_muK, label='SMICA', marker='o')
	plt.errorbar( x, np.array(Data[1][0])*K_to_muK,\
			 yerr=np.array(Data[0][1])*K_to_muK, label='SEVEM', marker='s')
	plt.errorbar( x, np.array(Data[2][0])*K_to_muK,\
			 yerr=np.array(Data[0][1])*K_to_muK, label='NILC', marker='p')
	plt.errorbar( x, np.array(Data[3][0])*K_to_muK,\
			 yerr=np.array(Data[0][1])*K_to_muK, label='Commander', marker='*')
	plt.title('$<{\Delta T}>$ around '+str(id_Sample)+' galaxies', loc='center', fontdict={'fontsize': 14} )
	...
	plt.savefig(fileProject+'graficos/CMBMeanT_on_'+str(id_Sample)+'Rnd_z0003_z'+str(Samp_choice)+'.png')
#	plt.savefig(fileProject+'graficos/CMBMeanT_on_Sa_z0003_z004_I2.pdf')
	plt.close()


def Rnd_ang(vec):	
	l_rnd = len(vec)
	costhrnd = np.random.uniform(-1,1, l_rnd)
	thrnd = np.arccos(costhrnd)
	phirnd = np.random.uniform(0,2*np.pi, l_rnd)
	return(thrnd, phirnd)
...
#	Vec_gxs = RextVec[1]			# The array contains pos+vector for each gxy in the sample.
	...
	for j in range( len(idMap) ):
		...
		sizes = np.linspace(5, 50, 10) 		#sizes = [5.0, 10.0, 15.0, 20.0, 25.0, 30.0, 35.0, 40.0, 45.0, 50.0]
		...
		for ss in sizes:
			...
			Tpix_disks = set()				# Set of values with T CMB from the pixels in disks 
			T_mean_rnds = []
			...
			print('# ------ CMB T values for pixels when begins the run in A_k'\
			      ' (should be a set {}) = ', Tpix_disks)	
			
			
			for g in range(0,Nrand):
				N_Disk_in_Mask = 0
				th,ph = Rnd_ang(Rext_gxs)
				# (This loop run over all the galaxies in the sample)
				for k in range( len(Rext_gxs) ):
					xrnd = np.sin(th[k])*np.cos(ph[k])
					yrnd = np.sin(th[k])*np.sin(ph[k])
					zrnd = np.cos(th[k])
...
#					R_disk_ext = (50.0)*arcsec_to_rad				# Radius of galaxy k in radians
					...
					Disk_k = hp.query_disc( nside=Nside, vec= (xrnd,yrnd,zrnd),\
				...
				# We check that all the pixels belong to the CMB maps and no one of them drop into the mask.
					if ( np.all( np.ones(len(Disk_k)) == T_mask[Disk_k] ) ):		
						Tpix_disk_k = T_data[Disk_k]				# Numpy array with T values in disk k.
						Tpix_disks.update(Tpix_disk_k)				# Set collecting the values of T.
				# If not we count the number of galaxies inside the mask.
					else:
						N_Disk_in_Mask = N_Disk_in_Mask + 1			
				
				print('')
				print('N_Disk_in_Mask', N_Disk_in_Mask)
				# We compute CMB T mean.
				T_val_rnd = np.array(list(Tpix_disks))			
				T_mean_rnd = np.nanmean(T_val_rnd)					# The mean T at A_k = ss for one Rnd sample.
				T_mean_rnds.append(T_mean_rnd)						# This array will have Nrand elements.
			
			T_mean_ss = np.nanmean(T_mean_rnds)
			T_std_ss = np.nanstd(T_mean_rnds)
			
		T_array.append(T_mean_ss)
		Std_array.append(T_std_ss)
		...
		if j == 0: # The final data for the CMB maps SMICA							
			...
			SMICA_Std_array = Std_array	
		elif j == 1: # The final data for the CMB maps SEVEM		
			SEVEM_T_array = T_array
			SEVEM_Std_array = Std_array
		elif j == 2: # The final data for the CMB maps NILC
			NILC_T_array = T_array
			NILC_Std_array = Std_array
		else: # The final data for the CMB maps Commander
			Commander_T_array = T_array
			Commander_Std_array = Std_array

# We save the dictionaries as pickle files
	if kk == 0: # ---- Sa Sample
		SMICA_Data = [ SMICA_T_array, SMICA_Std_array]
		SEVEM_Data = [ SEVEM_T_array, SEVEM_Std_array]
		NILC_Data = [ NILC_T_array, NILC_Std_array]
		Commander_Data = [ Commander_T_array, Commander_Std_array]
		
		dat = np.column_stack((SMICA_T_array, SMICA_Std_array, SEVEM_T_array,\
							   SEVEM_Std_array, NILC_T_array, NILC_Std_array,\
							   Commander_T_array, Commander_Std_array))
		hd_dat = " SMICA T          , SMICA Std       , SEVEM T          , SEVEM Std       ,\
		  NILC T           , NILC Std       , Commander T       , Commander Std    "
		np.savetxt(fileProject+'data/'+str(id_Sample[0])+'_Rnd.dat', dat, header=hd_dat,\
				  fmt=['%18.14e','%18.14e','%18.14e','%18.14e','%18.14e','%18.14e',\
					  '%18.14e','%18.14e'])
	...
	# The final data for the CMB maps SEVEM
		...
		hd_dat = " SMICA T          , SMICA Std       , SEVEM T          , SEVEM Std       ,\
		  NILC T           , NILC Std       , Commander T       , Commander Std    "
		np.savetxt(fileProject+'data/'+str(id_Sample[1])+'_Rnd.dat', dat, header=hd_dat,\
				  fmt=['%18.14e','%18.14e','%18.14e','%18.14e','%18.14e','%18.14e',\
					  '%18.14e','%18.14e'])
	...
	# The final data for the CMB maps Commander
		...
		hd_dat = " SMICA T          , SMICA Std       , SEVEM T          , SEVEM Std       ,\
		  NILC T           , NILC Std       , Commander T       , Commander Std    "
		np.savetxt(fileProject+'data/'+str(id_Sample[2])+'_Rnd.dat', dat, header=hd_dat,\
				  fmt=['%18.14e','%18.14e','%18.14e','%18.14e','%18.14e','%18.14e',\
					  '%18.14e','%18.14e'])
