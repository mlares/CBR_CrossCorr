...
def Plot_map(x, Data, id_Sample , ymax, ymin):
	...
	plt.savefig(fileProject+'graficos/RingCMBMeanT_on_'+str(id_Sample)+'_z0003_z'+str(Samp_choice)+'_ba0_05_30bins.png')
	plt.close()
...
#				R_disk_ext = (50.0)*arcsec_to_rad			# Radius of galaxy k in radians
			...
			# We realize that it is better to put inclusive=True.
				if x_Rext0 == 0:
					...
					if len(Ring_k) == 0:
						print('The code has broken down since the binning is to thin!'\
							  'Please check the parameter "n_bins" and increase it.')
			...
			print('')
			print('Percentage of the sky with data covered: ', len(list(Tpix_rings))/(0.78*Nside**2)  )
			print('################################################################')
